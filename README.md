...
- **Order execution** (`POST /api/v1/orders`, `internal/httpapi/order_handlers.go`) — places a real order against one of the authenticated user's connected brokers via `PlaceOrder`, gates Alpaca orders behind the PDT check first (OANDA/Questrade aren't subject to that FINRA rule), and persists successful orders per-user to Postgres. Failed submissions are never persisted. **This is the one endpoint that moves real money if the underlying broker credentials point at a live account** — see the operational notes below on how it was and wasn't tested.
...
- **Live price streaming** (`GET /ws/quotes`, `internal/httpapi/stream_handlers.go`) — an authenticated WebSocket endpoint pushing real ticks from OANDA's v20 streaming pricing API (`internal/brokerage/oanda`'s `StreamPricing`, on a separate `stream-fxpractice`/`stream-fxtrade` host from the REST API, with its own non-timing-out HTTP client since streams are long-lived). OANDA-only for now — Alpaca and Questrade don't have a streamer implementation yet. Verified against the real live OANDA feed: the connection stays open and healthy through heartbeats; the tick delivered was the last Friday-close price, since forex markets are closed weekends.
- HTTP API: auth (`/api/v1/auth/*`), broker credentials (`/api/v1/broker-credentials*`), balance/quote/orders, live quote stream (`/ws/quotes`), `GET /healthz`, `GET /logo.png` (see [Gateway HTTP API](#gateway-http-api))
- Web client (`web/`) — React + TypeScript + Tailwind, real login/register screens gating an eight-page shell (`/profile`, `/dashboard`, `/funds`, `/market/:symbol`, `/trade`, `/forecasts`, `/reports`, `/settings`). **Dashboard** (unified NAV, cross-border split, per-broker USD allocation donut, with an explicit "no brokers connected" empty state instead of a blank screen), **Settings** (broker connectivity audit), **Market data** (on-demand REST quote lookup, plus a real live-updating price stream for OANDA symbols), **Funds** (links to each broker's real funding portal), **Profile** (real user info + broker connect/disconnect UI), **Trade** (real order ticket, gated behind an explicit "this may execute a real trade" confirmation checkbox), and **Reports** (real order history table) are fully real, backed by the gateway. **Forecasts**, candlestick charting, L2 depth, and technical indicators render the correct layout but each explicitly states what backend piece it's waiting on, rather than showing placeholder/fabricated numbers.
...
- OHLC aggregation and candlestick rendering, Level 2 depth, and technical indicators (VWAP/EMA/Bollinger) — the live tick stream above is real, but nothing buckets ticks into bars, requests L2 data, or computes indicator math over the feed yet.
- Streaming for Alpaca and Questrade — only OANDA has a `StreamPricing` implementation so far.
- Automatic day-trade detection — `day_trades` rows aren't populated automatically from order fills yet (no same-day buy+sell round-trip matcher), so the PDT gate on Alpaca orders only sees trades some other process has already recorded there; it won't yet catch round-trips made through this app itself.
- TWAP/VWAP order slicing and RL-delegated execution routing — every order today routes as a single direct market/limit order.
...
**How order execution was and wasn't tested:** every validation path, the "broker not connected" path, the PDT-gate-triggers-an-account-fetch path, and broker-rejection paths were verified against real servers (Alpaca returns 401 on placeholder credentials, Questrade's `PlaceOrder` is a documented stub that always errors) — including confirming that failed attempts are never persisted to `orders`. The order-ticket UI's validation and confirm-checkbox gating were verified in a real browser. **The successful-order-placement code path was deliberately never exercised against a real broker**, because the only broker with working test credentials in this environment is a live OANDA account, and clicking "submit" there would execute a real trade. That path is verified by code review, not execution — if you want it exercised for real, use Alpaca paper-trading credentials (fake money, safe) rather than live OANDA.

**Operational notes if you're running this locally:**
- OANDA's `PlaceOrder` is fully implemented — if the `base_url` you save for your OANDA credentials is `api-fxtrade.oanda.com` (live) instead of `api-fxpractice.oanda.com` (practice), any code path that calls it executes real trades with real money. The web client's order ticket requires an explicit confirmation checkbox before submitting for exactly this reason.
...
│   │                              # broker credentials, OAuth tokens, orders, day trades
│   ├── engine/                  # Unified balance aggregation, PDT compliance
│   ├── httpapi/                 # HTTP handlers (auth, credentials, balance, quote, orders, health, logo)
...
| POST   | `/api/v1/orders`        | session | `{broker, symbol, side, type, quantity, limit_price?}` → places a real order via that broker's `PlaceOrder`. Gates Alpaca orders behind the PDT check first. **Moves real money if the broker connection is live** — see the operational notes above. |
| GET    | `/api/v1/orders`        | session | Returns the authenticated user's own persisted order history, most recent first. |
| GET    | `/ws/quotes?broker=&symbol=` | session | Upgrades to a WebSocket and pushes live ticks from the broker's streaming API. OANDA-only today. Session cookie is checked before the upgrade; failures are plain HTTP errors, not JSON. |

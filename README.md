...
- Real-time streaming ingestion pipeline (`cmd/data_pipeline/`) — Kafka/WebSocket market data fan-out, technical indicator computation over a live tick buffer. Blocks: Market page's candlestick stream, Level 2 depth, indicator overlays. (Market data quote *lookups* are real and live today; there's just no streaming tick feed yet.)

...
-- Orders are scoped to the user who placed them, not a locally-cached
-- "account" row — account data (equity/cash/buying power) is always read
-- live from the broker (see internal/userbrokers), never persisted here.
...
-- day day-trade count for margin accounts under $25,000 USD equity. Nothing
-- populates this automatically yet (same-day buy+sell round-trip detection
-- isn't built) — the PDT gate in POST /api/v1/orders reads whatever rows
-- exist, so it won't yet catch trades made outside this app.
...
CREATE INDEX IF NOT EXISTS idx_day_trades_user_date ON day_trades (user_id, trade_date);
